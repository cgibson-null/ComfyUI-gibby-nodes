    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.String.Input("mask_indices", default="", tooltip="Which indices to process, e.g. 0-2, 5, 7 (a range includes both ends). Ignores indices >= the count. Empty=all. mask_mode=split: the mask region indices. mask_mode=single with a batch: the batch item indices (each its own crop)."),
